...
export default class WaterfallFlow extends React.PureComponent {
  ...
  _computePositions = (byRender) => {
    ...
    const listHeaderComponentArrayLen = ListHeaderComponentArray.length;
    const columnHeights = []
    const originItems = []
    // 计算头部元素
    ListHeaderComponentArray.forEach((item, index) => {
      const offsetTop = columnHeights[0] || 0
      // const height = itemHeight({ item, index })
      const height = this._itemHeights[index]
      // 头部渲染的item，columnHeights都需要加上
      for (let idx = 0; idx < numColumns; idx++) {
        columnHeights[idx] = offsetTop + (height || 0)
      }
      const obj = {
        height,
        columnIndex: -1,
    ...
    data.forEach((item, index) => {
      ...
      // const height = itemHeight({ item, index })
      const height = this._itemHeights[index + listHeaderComponentArrayLen]
  ...
  render() {
    ...
    this._itemHeights.length = ListHeaderComponentArray.length + data.length
    ...
    return (
      <FlatList
        ...
        renderItem={({ item, index }) => {
          if (index <= ListHeaderComponentArray.length - 1) {
          ...
          return (
            <WaterfallItem
              ...
              onItemHeightChange={(height, index) => {
                index = ListHeaderComponentArray.length + index
                const preHeight = this._itemHeights[index]
                if (this._itemHeights[index] !== height) {
                  const preAllLoaded = !JSON.parse(JSON.stringify(this._itemHeights)).some(o => o === null)
                  this._itemHeights[index] = height
